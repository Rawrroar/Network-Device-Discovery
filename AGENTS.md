...
- Onboarding: `OnboardDiscoveredDevicesJob` + `@action onboard` on `DiscoveredDeviceUIViewSet` (GET form / POST enqueues job via `JobResult.enqueue_job`). Custom UIViewSet action URL names use underscore format: `discovereddevice_onboard` (not `-onboard`). JobResult detail redirect is `extras:jobresult` (NOT `jobresult_detail` — that route does not exist in 3.2.x). List-view button comes from the plugin's `discovereddevice_list.html` template override of the `bulk_buttons` block.
...
- Models without any unique field/constraint (`DiscoveryScan`, `DiscoveryResult`) MUST declare `natural_key_field_names = ["pk"]` as a CLASS attribute (not in `Meta` — Meta rejects it) or their detail pages crash with "Unable to identify an intrinsic natural-key definition".
